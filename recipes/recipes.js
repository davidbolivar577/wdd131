...
let displayList = [recipes[Math.floor(Math.random() * recipes.length)]];
...
function addRating(r){
    let html = `<span class="rating" role="img" aria-label="Rating: ${r} out of 5 stars">`;
    for(i = 0; i < 5; i++){
        if(i < r){
...
function render() {
    ...
    displayList.forEach(current => {
        const recipeDiv = document.createElement('div');
        recipeDiv.className = 'recipe';

        let html = `
                <img class="image" src=${current.image} alt=${current.name}>
                ${addCategories(current.tags)}
                <span class="name">
                    ${current.name}
                </span>
                ${addRating(current.rating)}
...
function search() {
    ...
    displayList = filteredRecipes.sort(compareRecipes);
